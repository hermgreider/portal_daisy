...
void StringSynth::Init(float sr) 
{
    samplerate = sr;
    cutoff_base = 1200.0f;
...
void StringSynth::Update() {}    
void StringSynth::Select1() {}
void StringSynth::Select2() {}
void StringSynth::Select3() {}

void StringSynth::InitVoices() 
{
...
void StringSynth::InitVoice(Voice &v) 
{
    v.freq = 0;
    ...
    v.amp_env.SetTime(ADSR_SEG_ATTACK, 1.0f);
...
void StringSynth::Process(float &outL, float &outR)
{
    float mix = 0.0f;
...
void StringSynth::SetCutoffBase(float val) 
{
    cutoff_base = fminf(fmaxf(val, 100.0f), 8000.0f);
}

void StringSynth::AdjustCutoff(float delta) 
{
    SetCutoffBase(cutoff_base + delta);
}

float StringSynth::GetCutoffBase() const 
{
    return cutoff_base;
}

void StringSynth::NoteOn(NoteOnEvent m)
{
    voices[current_voice].NoteOn(m.note);
    current_voice = (current_voice + 1) % kNumVoices;
}

void StringSynth::NoteOff(NoteOffEvent m)
{
    for(int v = 0; v < kNumVoices; v++)
    {
